#plot phylogeny with internal nodes labeled, as well as major clades
library(phylobase)
library(ape)
library(phytools)
library(philr)
...
#WGD, down, and up nodes were all determined from the Angiosperm_Medusa_tree
...
for(i in 1:length(down_nodes))
  nodelabels("d", down_nodes[i], frame="circle" ,col="blue", bg="blue", cex = 0.3)
for(i in 1:length(up_nodes))
  nodelabels("u", up_nodes[i], frame="circle" ,col="red", bg="red", cex = 0.3)	
...
mytree$node.label
WGD_nodes = c(7877,9897,7873,7864,9622,9700,9654,12054,10163,12097,12117,12118,10208,10233,10333,10277,11485,11280,10637,10708,10483,10567,10432,12963,12993,12858,12791,13452,13661,13497,13808,14394,14509,14580,15105,15166,7860,9649,9842,10010,12629,10181,10176,11947,11982,12853,12771,13319,13497,15502,10135,11356,9628,7995,9279,9073,7884,7938,8005,8923,8857,8864,8036,8345,8166,8201,8296,9930,9846,9685,12078,12096,11113,11134,11548,11428,11490,11237,10663,10440,12730,12718,12800,13760,10132,15206,8000,8050,8105,8192,11195,9623,9074,8311,13097,13155,15204,13096,7996,15175)
listofnodes <- (1:(7858+7859))
listofnodes <- cbind(listofnodes, rep(NA,(7858+7859)))

listofnodes[,2][listofnodes[,1] %in% WGD_nodes] <- "P"

mytree$node.label <- listofnodes[7860:15717,2]
...
mydata
mydata$Species
genera1<-sapply(strsplit(as.character(mydata$Species),"_"),function(x) x[1])
genera1

mydata2 <- read.csv(file = "typespecies.csv")
mydata2
mydata2$Type_Species
genera2<-sapply(strsplit(as.character(mydata2$Type_Species),"_"),function(x) x[1])
genera2

generaall <- union(genera1,genera2)
tipstoremove <- setdiff(mytree$tip.label,generaall)
tipstoremove
...
pdf("Angiosperm_Medusa_and_WGD2.pdf",width=18, height=18)
plot(mytree,type="fan", font=1, adj=0.5, no.margin=TRUE, edge.width=0.5, show.tip.label=TRUE, cex=1)
nodelabels(mytree$node.label, cex=2)
dev.off()

pdf("Angiosperm_Medusa_and_WGD3.pdf",width=6, height=18)
plot(mytree,no.margin=TRUE, edge.width=0.5, show.tip.label=TRUE, cex=0.8)
nodelabels(mytree$node.label, cex=1)
dev.off()

mytree$edge.length <- NULL
pdf("Angiosperm_Medusa_and_WGD4.pdf",width=6, height=18)
plot(mytree,no.margin=TRUE, edge.width=0.5, show.tip.label=TRUE, cex=1)
nodelabels(mytree$node.label, cex=2)
dev.off()
